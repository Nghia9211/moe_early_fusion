...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, queries, keys):
        ...
        matmul_output = torch.bmm(Q_, K_.transpose(1, 2)) / (d_k ** 0.5)  # (h*N, T_q, T_k)
...
class SASRec(nn.Module):
    ...
    def forward(self, states, len_states):
        inputs_emb = self.item_embeddings(states) * self.scale
        inputs_emb += self.positional_embeddings(torch.arange(self.state_size).to(self.device))
        seq = self.emb_dropout(inputs_emb)
        mask = torch.ne(states, 0).float().unsqueeze(-1).to(self.device)
        seq *= mask
        seq_normalized = self.ln_1(seq)
        mh_attn_out = self.mh_attn(seq_normalized, seq_normalized)
