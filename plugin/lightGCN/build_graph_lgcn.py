...
def build_graph_with_masking(user_file, item_file, review_file, gt_folder, output_file):
    ...
    # ----------------------------------------------------------------
    # LightGCN paper (He et al., 2020): Chuẩn hóa theo công thức
    # A_hat = D^{-1/2} * A * D^{-1/2}
    # KHÔNG cộng identity matrix (không self-loop).
    # Self-loop được xử lý tự nhiên thông qua layer combination E_final = mean(E0..EK).
    # ----------------------------------------------------------------
    ...
    # Không cộng I — đây là điểm khác biệt quan trọng so với GCN thông thường
    A_csr = A.tocsr()
